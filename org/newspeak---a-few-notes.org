...
todo-00-last IN-PROGRESS-NOW : add section on popup menus:

When we want to add a popup menu attached to an image to UI:
1. In ~#definition~ add call to ~#Presenter#dropDownMenu:[#Presenter#menuWithLabelsAndActions: {{'Save'.[respondToSave]}}]~, for example:
   ...
   #+end_src
   1. The passed menu item -> action may look like this
     ...
     #+end_src   
2. Add the respond actions, for example:
...
What happens during the UI creation and composition?

1. During UI composition, this is the Fragment that is created by the call to ~#definition~ which calls ~dropDownMenu: [classActionsMenu]~
   #+begin_src smalltalk
     dropDownMenu: menu <[Menu]> ^ <DropDownMenuFragment> = (
	     ^DropDownMenuFragment menu: menu
     )
   #+end_src
2. Which adds a ~DropDownMenuFragment~ into the UI
   #+begin_src smalltalk
   ...
   #+begin_src smalltalk
     class ImageButtonFragment = LeafFragment (
       ...
       |)
   #+end_src
3. The above ties to the Fragment an ~image~ and empty ~action~; I do NOT know when they are set.


*When the UI is composed as a Newspeak object graph of the ~Fragment~ s instances, AT SOME POINT (which point??) THE NEWSPEAK Fragment graph is converted to Javascript DOM document (Javascript objects graph). This process calls ~#createVisual~ on every ~Fragment~ (How and where in code does this composition and calling createVisual happens??)* During this process, each ~Fragment#createVisual~ does BOTH create a DOM elements from the Fragment AND adds listeners to them, as directed by the code in ~#createVisual~.

For the ~DropDownMenuFragment#createVisual~ this is executed:

1. ~DropDownMenuFragment~ is converted to DOM element and DOM element Listeners are created on 'click', 'mousedown', mouseup etc :

   #+name: DropDownMenuFragment#createVisual
   #+begin_src smalltalk
     createVisual ^ <Alien[Element]> = (
       | menu = super createVisual. | (* calls ImageButtonFragment which creates an img element *)
       ...
       menu addEventListener: 'click' action: [:event | 
           isActive 
             ...
             ifFalse: [updateContent].  (* HERE, SELF=VISUAL=button=DropDownMenuFragment *)
           nil
         ].
       (menu at: 'style')
...
2. The 'click' event listener installed above in ~DropDownMenuFragment#createVisual~ is executed. This code is (again)
   #+begin_src smalltalk
      [:event | 
           isActive 
             ifTrue: [isActive: false. shell closeActiveMenu] 
...
3. The  ~DropDownMenuFragment#updateContent~ is executed (again described in Newspeak code, although this runs in JS DOM). *This is the core leaf process that happens after the image click.*
     ...
     #+begin_src smalltalk
       updateContent ^ <Alien[Element]> = (
         ...
         (* HERE:
             menu = the popup menu DOM element created by computeContentForMenu: menuSupplier 
             visual = the button = aDropDownMenuFragment visualX = the result of DropDownMenuFragment#createVisual
         ,*)
         ^menu.
       )
     #+end_src
   - The code explained in summary below, in detail in items 4. and 5. 
     ...
     computeContentForMenu: menuSupplier <[Menu]> ^ <Alien[Div]> = (
     	...
     	menuSupplier value do: 
     		[:menuItem <MenuItem | Symbol> |
     		 ...
     		 itemContent:: contentFor: menuItem within: dropDownContent. (* This is another huge DOM creation, only described below *)
...
5. The code that *shows the menu (the visual=aDropDownMenuFragment) in shell and positions it in the shell*. 
   ...
   #+end_src
   - In detail:   
   - ~menu~ element is appended as child of the ~visual~, which is instance of the ~DropDownMenuFragment~!!
   - ~menu~ element is positioned in the DOM (the ~shell~)
   - ~menu~ element is set as activeMenu on ~shell~
   - ~visual~ is set as parent of the active menu in ~shell~  
...
We recall that the menu items are DOM elements created in  method ~HopscotchForHTML5#contentFor:within:~
...
That code have run already in ~#computeContentForMenu~ and have created the popup menu, menu items, and added listener on mouse 'click' with action from the ~Presenter#menuWithLabelsAndActions~ 

*So now, if user clicks on one of the menu items, the action from  ~Presenter#menuWithLabelsAndActions~ runs. THIS CONCLUDE THE WHOLE LIFECYCLE OF MENUS!!!!*
...
- There is NO ClassDeclaration or MixinDeclaration or SlotDeclaration, only ClassDeclarationMirror, SlotDeclarationMirror
- *THERE IS A SORT-OF-CIRCULARITY BETWEEN  ~ClassMirror~ AND ~ClassDeclarationMirror~, WE CAN GET ONE FROM THE OTHER AS FOLLOWS*
  ...
  - ~aClassDeclarationMirror instanceSide        => MixinMirror similar to ClassMirror~ *MixinMirror has similar API to ClassMirror, it provides slots <MirrorGroup>, nestedClasses <MirrorGroup>, methods <MirrorGroup>* .. *note there is also ~#definingMixin~ method which results in the enclosing instance.* 
