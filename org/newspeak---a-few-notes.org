...
*Originally intended as a blog, for now, keep as a normal document*
...
In more detail, let us try the following (skip if you know Smalltalk basics):
...
- We are now using Newspeak, in it's full power.
...
- About the changes you make:
  - Any changes we make (for example: add a class, change a class, evaluate something in the workspace) are stored in the browser local storage in regular intervals.
...
- Action: On the top right of the page (in the same line where we see "Root +", click the vertical 3-dot link [[file:img/newspeak---a-few-notes.org-3-vertical-dots.png]]
...
- Result: *The classes from the selected ~.ns~ files now show in your IDE.* It should look like this [[file:img/newspeak---a-few-notes.org-counter-classes.png]]
  - Note 1: We have "imported" the code of the  ~CounterApp.ns~ and ~CounterUI.ns~ classes to Newspeak by running "Compile file". Normally, we would add the classes through the IDE by clicking on the "+" button [[file:img/newspeak---a-few-notes.org-root-plus-button.png]]. We choose to "import" files at the moment to concentrate on the process, not the code.
   
  - Note 2: Click on the ~CounterApp~ or ~CounterUI~ link. This shows the code of the corresponding class "source code". 
  - Note 3: The ~CounterApp~ shows links to ~[deploy] [configuration] [run] [debug]~. Why only the "app" classes such as ~CounterApp~ (and no other classes) do show the ~[deploy] [configuration] [run] [debug]~ links in the Newspeak IDE? That is because the "app" classes represent a runnable Newspeak application.
...
- How to download, install, and start the *local Newspeak server*.
- That pointing a browser to http://localhost:8080/primordialsoup.html?snapshot=HopscotchWebIDE.vfuel runs the local version of Newspeak IDE (in the browser)

Assumptions and notes: 

- Python 3 needs to be installed on our system.
...
- *However, apps can NOT be deployed to the online version (there are no facilities for it. Maybe there should be?). We can do the coding online (as described above), but the actual deployment has to be done into a Newspeak system we run locally.*
...
** Simle methods to install and run Newspeak

*** <<install-method-1>> Install method 1: No installation or setup, run Newspeak online (*Recommended for a quick start*)

TL;DR: This section describes the simpliest setup - in fact, this method requires no setup. 
...
How would I go by building and running a "Hello world" application? I know I can create a class, in the browser IDE .. ok .. but from there, how to I "create and deploy the application" so I end up with something like the ToDo app ... hmm, I should probably do my homework and study it, but any high level notes on that?

*Two options to build an app*

1) Easy but flakey. There is a 'deploy' option that the IDE displays for apps.  It's buggy and slow, but mostly works if you are prepared to wait a ridiculous amount of time, and tolerate the fact that the app will be too large due to bugs in the serialization code :-(. I need to fix that, but you can deploy apps that way, especially if they are small. You choose the option 'asVictoryFuelWithMirrors' if your app uses the GUI (I think there's some mirror dependency in the UI)  and 'asVictoryFuel' otherwise.
