...
* Changelog: last change 2023-09-28
...
| 2023-09-21 |   | Milan Zimmermann |   | Added sections [[* 2023 Update]  [[* Newspeak: Script which builds deployable vfuel files]  [[* Ampleforth and HTML documents in Newspeak]  todo-00 finish and double-text the headings                                                                                                    |



* Todos todo-00

 - change begin_example to begin_src??

 - above, add ".ns" to class names
 ...
 - Look for App, replace with Application, except code . Same for Lib -> Library.

 - Look for Application, Library, TestConfiguration, Test
   
 - generally replace 'are called' 'abc is called' with 'are termed', 'abc is termed'
     
 - In [[* Newspeak modules API zoo][Newspeak modules API zoo]]:
   - research the roles /build/ and /instantiate/ : Role=build(Library), execute(Application) in ~#buildUsing: platform~
   - todo-00 : add this somewhere, and use investigation in Zoo : Instances and their production: Clarify that class declarations create an object termed 'class factory object' that provides the means of producing instances of the class. The factory object supports at least one message that produces new instances (of the class). This message on the 'class factory object' is termed the 'primary factory method'.  The 'class factory object' in code, is referenced using the uppercase class name. 
     - So code such as ~AMyApp packageUsing: manifest~ invokes the factory method ~#packageUsing~ on the 'class factory object' ALSO named ~AMyApp~, and it produces an instance of the ~class AMyApp~.  We can use use it in code also as ~myAppInstance = AMyApp packageUsing: manifest~
     - todo-00 : *Specifically for modules (Application, Test, Library)*:
       - 1. invoking ~#packageUsing manifest~ and similar named primary factory methods: They are TYPICALLY called to produce instances, but somethimes MAYBE ??? for side effects??? (the side effect would be creation of a deployable artifact).
         - Examples producing instance:
           - Here we just produce instance of hopscotchIDE on slot (note: instance, despite the often used uppercase name on slots, here HopscotchIDE)
             #+begin_example
       ...
       - 2. invoking ~#buildUsing: platform~ and similar named instance methods: Are they ususally called to return instances? Often they seem to do, returning ~SomeConcreteExtension>>#buildUsing: platform~ But we should NOT call the role /instantiate/!!! 
...
Later sections decribe the process of bringing dependencies into a Newspeak programs. We use the /manifest/ passed to module APIs, from the manifest, dependency classes are stored on slots ("imported"), later picked up from slots when needed to instantiate.  [[*Modules: Application, Library, TestConfiguration, General][Modules: Application, Library, TestConfiguration, General]], the recipy section [[*Newspeak recipy for writing modules API][Newspeak recipy for writing modules API]], and the [[*Hello World in Newspeak][Hello World in Newspeak]] and other sections are largely about bringing dependencies into modules and objects.
...
TL;DR: ~manifest~ is needed for packaging, ~platform~ is needed at runtime. More details are below, and in the section [[*Modules: Application, Library, TestConfiguration, General][Modules: Application, Library, TestConfiguration, General]].  
...
2. A Library must be able to be *instantiated, loaded or linked, then used*, at least in principle, by the caller Application or another Library on that platform.  The library /used/ by an Application or another Library is often called a dependency.  The term /used/ implies that the Library is *found* and *connected* to the Application or Library which is using it.
3. In addition, we should be able to *package* both the Application and the Library for distribution (deployment).
...
- For an App:
  - To perform /role/ 3, /packaging/, let's call the method ~#package~.
  - To perform /role/ 1, /start execution/, let's call the method ~#main~
- For a Library:
  - To perform /role/ 3, /packaging/, let's call the method ~#packageLibrary~
  - To perform /role/ 2, /instantiate, load and use by the caller app/, let's call the method ~#build~
...
- *Newspeak Application module* is defined by the presence of:
  - Primary factory method ~#packageUsing: manifest~ which performs /role/ 3, /packaging/. 
    - Implementations should set pull needed classes from the manifest and place then on slots to package them on the app artifact.  Application slots serve as "imports".
  - Instance method ~#main: platform args: args~ which performs /role/ 1, /start execution/.
    - Implementations should instantiate, from slot classes and platform objects, all object needed to run the app, then call methods on them which perform "running the app".  Examples of needed objects instantiating:
      - /Slot1 usingPlatform: platform [and: otherObjects]/ if Slot1 is a General module class
      - /Slot2 buildUsing: platform/ if Slot1 is a Library module class

        todo-00 : finish going over (build) and similar methods
- *Newspeak Library module* is defined by the presence of:
  - Primary factory method ~#packageLibraryUsing: manifest~ which performs /role/ 3, /packaging/.
  ...
  - Instance method ~#buildUsing: platform~ which performs /role/ 2, /instantiate, load and use by the caller app/.
    - Implementations should build, then return a working instance of the module we want to distribute, NOT the instance on which this #buildUsing is defined!! Building the distributed instance will possibly call other needed dependencies ~#buildUsing: platform~ or ~#usingPlatform: platform~ methods.  Examples of how needed dependencies :
      - /Slot1 usingPlatform: platform [and: otherObjects]/ if Slot1 is a General module class
      - /Slot2 buildUsing: platform/ if Slot1 is a Library module class
...
- *Newspeak TestConfiguration module* by convention ends with "Configuration", and is defined by the presence of:
  - Primary factory method ~#packageTestsUsing: manifest~ which performs /role/ 3, /packaging/
...
|                                          | Roles=instantiate + package(import dependencies)      |                               |     | Role=build(Library), execute(Application),      |                                                                 |
|------------------------------------------+-------------------------------------------------------+-------------------------------+-----+-------------------------------------------------+-----------------------------------------------------------------|
...
|--------------------------------+-----------------+---------------------+----------------------------+-----------------|
| #packageLibraryUsing: manifest | primary factory | - role is package - | #packageUsing: manifest    | primary factory |
...
By now, we know that /modules/ are /instances of top level classes/, see [[*Discussion of Modules][Discussion of Modules]].  In [[*Newspeak modules API summary][Newspeak modules API summary]] we have shown that when modules are given "convention methods", modules can uniformly behave as Apps, Libraries, or TestConfigurations - on behalf of other modules, and IDEs.  This is enabled by uniform understanding of the "convention methods" by the Newspeak ecosystem.

The [[*Newspeak modules API summary][Newspeak modules API summary]] describes the API for a module to be able to behave as an Application on the Newspeak platform. 

So to create a Newspeak Application (as opposed to a general module or a library), we need different API. Here are the steps to create the ~HelloWorldApp~ *application* in the Newspeak IDE's
...
- As shown in the previous section, click the "+ ", paste the following class to the IDE, and click the "Accept" button.
  ...
  #+end_src
- You should see the class in the top classes list.  However, (as opposed to previous section ~HelloWorldGeneralModule~), our class has the links *[deploy] [configurations] [run] [debug]* beside it.  This is because the presence of ~#packageUsing: manifest~ factory makes the class an Application by convention.  The tools (the IDE) understand this message and use it to show actions that can be done with an App: run, debug, deploy, show available configurations.  In addition, the presence of another method, the ~#main: platform args: args~ makes the module runnable as a standalone App. 
...
Let's pretend our Application can use a Newspeak module produced by a 3rd party (3rd party to us). Call the 3rd party the /TranslatorCorp/. Let's pretend /TranslatorCorp/ provides the module ~HelloTranslator~, packaged as ~HelloTranslatorLib~.
...
As shown in the previous section [[*3. Hello World App][3. Hello World App]], we can both run the Application from the IDE, or package it a ".vfuel" file, and distribute to run as a standalone Application from a local Newspeak webserver (or as an Electon based on Android, iOS, or desktop, but this is not shown yet).
...
Imagine a convention whereby *every library intended for distribution is sent out as a class which:*
 ...
 - has a (build) ~#usingPlatform: platform~ method - this method, given a platform object, produces a working instance of the module we actually want to distribute
...
*** Code the app
The process of coding (creating) an app is described in [[*  Build, run, and debug the CounterApp in Newspeak (in the online version)][ Build, run, and debug the CounterApp in Newspeak (in the online version)]]. This process is also described more succintly in the 'Hello World' programs section [[*3. Hello World App][3. Hello World App]].
...
TL;DR: This section is a code-browsing tour which discovers Hopscotch, discovering the Hopscotch MVC: the *Subject*, *Model*, *Presenter*, as well as *Fragment*, the class that translates Hopscotch classes to HTML. At the end, in [[* Using chapters above to write a simple Hopscotch UI App, the *CheckedItemApp*][we use the acquired insights to write a sample App, the *CheckedItemApp*]]. 
...
- The end goal of our process is to write a simple Newspeak Application with UI in Hopscotch. This simple Newspeak Application is developed in chapter [[*Using chapters above to write a simple Hopscotch UI App, the *CheckedItemApp*][Using chapters above to write a simple Hopscotch UI App, the *CheckedItemApp*]].
...
The last line will create instance of ConcretePresenter by calling ~ConcreteSubject>>#presenter~ and open it in a new window where the Application will live! This snippet does not show definitions of ~ConcreteModel~ ~ConcreteSubject~ ~ConcretePresenter~, or how we obtain the ~hopscotchWindow~. We will get to that when creating a concrete Application in [[*Using chapters above to write a simple Hopscotch UI App, the *CheckedItemApp*][Using chapters above to write a simple Hopscotch UI App, the *CheckedItemApp*]]
...
The above steps should be sufficient to write a basic Application with Hopscotch UI, which we will do in later chapter [[*Using chapters above to write a simple Hopscotch UI App, the *CheckedItemApp*][Using chapters above to write a simple Hopscotch UI App, the *CheckedItemApp*]].
...
This chapter goal is to use the intuition and knowledge acquired in the above chapters, to write a sample UI Application in Newspeak. 
...
To install the complete code to Newspeak, download the ~.ns~ files from the above link, then, from your Newspeak IDE (local or online) use the "Compile file(s)" link to import the files into your IDE. If you are not sure what that means, read the section around [[compile-files]["Compile file(s)"]].
...
Note: The code for the *CheckedItemApp* built here is as simple as it can be, to focus on understanding ~HopscotchForHTML5~. The Application should be improved in the future. The main weakness of the current version is that it exposes model publicly - model functions should be wrapped in Subject.
...
TL;DR: Application is a top level class supporting the message ~#packageUsing: manifest~, as explained in 
This section browses code following code execution after we click [deploy] on an 

Creates Application package instance, runs it's main: to initialize it, then calls serialization on the initialized instance
- this is definitely NOT calling primary factory for side effects as I thought!!!
...
public bytesForVictoryFuelWithMirrors ^ <ByteArray> = (
  (* this becomes: bytesForVictoryFuelWithRuntime: (HopscotchWebIDE RuntimeWithMirrorsForPrimordialSoup)
...
public bytesForVictoryFuelWithRuntime: runtimeClass ^ <ByteArray> = (
        ...
        (* ****** This is where low-level serialization happens for the app that has "packageUsing: manifest".
           THIS CALLS THE PRIMARY FACTORY ON appDef, which creates instance, then converts to bytes - this is the .vfuel file created 
         *)
...
public packageApplicationConfiguration: appDef <Class> withRuntimeConfiguration: runtimeClass <Class> usingNamespace: ns  ^ <ByteArray> = (
    ...
    runtime:: runtimeClass packageRuntimeUsing:
        ...
        (* THE PACKAGING OF appDef - the application class - is done here *)
        (* THIS IS IN out/WebCompiler.ns : HERE THE Preloader DOES FIRST CREATE PACKAGE INSTANCE, THEN CALLS THE PACKAGE MAIN:
           WHICH RUNS THE PACKAGE , THEN THE MAIN RETURNS THE PACKAGE INSTANCE (!! BY DEFAULT) - THE MAIN: ALSO INITIALIZES THE PACKAGE INSTANCE (??)
