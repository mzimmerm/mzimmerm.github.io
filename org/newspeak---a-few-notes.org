...
11. From the last list, we can see those are all UI elements- checkboxes, pickers, radio buttons etc. Their concrete ~createVisual~ implementations use the document objecrt to create a representation of UI elements which are clearly equivalent to DOM object, and can be converted to DOM objects. Let's look at ~CheckboxFragment#createVisual~ as an example:
...
- The ~#helpText~ contains a (almost) pure HTML and ~mappings~ of special class names such as ~hopscotchClearResultsButton~, ~classExpander~, etc.
    #+begin_example
      mapping 
        ...
        .. etc ..
    #+end_example
- We saw the ~#helpText~ returned an instance of class is ~HopscotchForHTML5>>Fragment~
- The returned ~HopscotchForHTML5>>Fragment~ instance then applies on each value from ~mapping~ the ~HopscotchForHTML5>>Fragment>>#createVisual~ at the end of the chain - building the DOM-like objects (called Fragments). Those visual instances are then inserted in the DOM, putting together the Newspeak IDE!. 


*Conclusion of sequence of calls:*

We followed the sequence of method calls after the "Help button" was clicked:

- ~Browsing>>ObjectPresenter>>#helpText~                             - calls Presenter self in:
- ~HopscotchForHTML5>>Presenter>>#ampleforth:mapping:~               - this Presenter creates a Fragment ~^AmpleforthFragment html: h mapping: m~ in:
- ~HopscotchForHTML5>>AmpleforthFragment class>>#html:mapping:~      - This AmpleforthFragment constructor does the core of the DOM-like child fragments creation in:
- ~HopscotchForHTML5>>AmpleforthFragment>>#processMappings~          - For each mapping, one visual is created in the calls below
- ~HopscotchForHTML5>>Fragment>>#visual~
- ~CheckboxFragment#createVisual~                                    - This uses "document createElement" to create and return an Alien DOM <div> equivalent - it contains a DOM element such as a checkbox.

We saw how this sequence is responsible of presenting Help Text (with embedded Newspeak objects) as an navigatable live HTML, where we can explore and drill in instances, classes, or other Newspeak artifacts.
