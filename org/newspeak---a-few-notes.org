...
2) Wheather the accessed items are declared in the same class, nested class or enclosing class of the expression for which the rules evaluate access. In other words, access control rules interact with the lexical structure. 
...
Clearly, access control rules are not trivial.
...
- Newspeak classes can have the following members: *slots, methods, inner classes*. 
...
- Because in Newspeak runtime, any operation is a message send, we can speak of the following: 
   ...
   - If objA can reference objB, we say that objA has access (or access rights) to objB. Further, if objA has access to objB, assume there is a method, slot, or nested class named ~member~ declared on objB. Even though objA has access to objB, objA still may or may not be able to send the message ~member~ to objB (execute ~objB member~). If objA has the ability to send message msg to objB, we say that "object objA has access to member on objB"
   

Combined, we say that objA has *access* (or *access rights*) to send a message ~member~ to objB, if a message send ~objB member~ is allowed to execute at runtime from some place in code of objA.

  
*** Slides 61, 62, 63: Rules for accessing members above and below the class in the nesting hierarchy

This section goes over access control rules described in slides in [[https://00463064267054129163.googlegroups.com/attach/16b8a25058caa/ns-access-control.pdf?part=0.1&view=1&vt=ANaJVrHvttkWPn-fNP95UFKbcCbbp1VaDJlJSx1HzBevq_xhafVpZ4uz1ffKMTvkAxLFnAk2_xurwU-BTi3FIMnPj1FlU7-S-zKgS2y2eNXheQQsxM8mJUY][Can Beautiful Languages Do Access Control?]].

**** Slides 61, 62, 63 overview 

These slides describe how Newspeak code in a class accesses members (classes, slots, methods) above and below the class in the nesting hierarchy and the member members, depending on the accessed member's public, private and protected status. In particular:
...
**** Colollary on slides 61, 62, 63

Public members are accessible everywhere, including from modules outside of modules where they are declared. 


**** Slides 64-77: Types of message sends

This section makes a few notes about slides 64-77 in  [[https://00463064267054129163.googlegroups.com/attach/16b8a25058caa/ns-access-control.pdf?part=0.1&view=1&vt=ANaJVrHvttkWPn-fNP95UFKbcCbbp1VaDJlJSx1HzBevq_xhafVpZ4uz1ffKMTvkAxLFnAk2_xurwU-BTi3FIMnPj1FlU7-S-zKgS2y2eNXheQQsxM8mJUY][Can Beautiful Languages Do Access Control?]].

Those slides describe types of message sends in Newspeak, as well as the algorithms of how the appropriate messages are looked up due to nesting, inheritance, and presence of access modifies on messages. 

A message is denoted as *msg* in this text. However, keep in mind that a *msg* can be a name of any class *member* (a *slot*, a *method*, a *nested class*). This unification is possible due to the fact that every runtime operation in Newspeak is a message send - so accessing a slot, a method, or a nested class can be all represented as a message send: Either ~anObject msg~ or ~msg~. The latter (~msg~) is a message sent to an implicit receiver. In the former (~anObject msg~) /anObject/ can be a special object /super/ or /self/ or /outer/, but the two basic forms of sending a message are all that we need to think of.

Syntactically, the types of message sends can be expressed as one of the following -  

- ordinary sends (*anObject msg*), 
- self sends (*self msg*), 
- super sends (*super msg*), 
- implicit receiver sends (*msg*), 
- outer sends (*outer Classname msg*)

See slides 64-77 for details of the algorithms. The text below only has a few selected comments and questions/answers regarding those slides.

1. These slides describe algorithms of how Newspeak finds the class on which the message exists.  It is the found class's message which is send (to the receiver object). One interesting thing is that in protected lookup, existence of a private msg is ignored.

2. A slot access is a subset of the implicit receiver send. As an example, let’s assume in code, there is a line "slot1." or "slot1:: 10." (outside the slot declaration section).  The rules for "implicit receiver sends" described in slides 69-77, apply for "slot1" - just substitute "msg" for "slot1".  Same situation applies for accessing nested classes - because a "msg", a "slot" a "nested class" are all "members", and all the rules apply for members.

3. On slide 69, implicit receiver send: "Lookup declaration of private msg in immediately surrounding class. If not found, recurse up lexical scope until lexical top." : 

   - Question: What does the "recurse up lexical scope" look for? Does it look up the lexical scope only for private msg? Also if a public or protected msg is found lexically up, is such msg skipped OR does a presence of public or protected msg trigger the inheritance chain lookup, even though there may be lexically higher level private msg?

   - Answer: Any lexically visible declaration will do. The point here is that if it's private, you can run the code immediately. If it is not private, it may have been overridden in a subclass, so you need to do the lookup. If it hasn't been overridden, you will eventually hit the definition you saw in the lexical scope.
  
4. Slide 68 'enclosing objects' shows circles with C0, C1 etc. 

  - Question: what are the circles with C0, C1 etc? The classes are S0 ,S1 etc. I assume the O0, O1, O2,etc are instances of S0, S1, S2 et.  Do the C0, C1, C2 etc circles have something to do with showing the fact that classes are virtual in Newspeak?
  - Answer: Yes. The Ci are classes. The slide illustrates what can happen in the general case.


*** IN-PROGRESS-NOW todo: Worked accessibility examples of for slides 62 and 63

**** Worker accessibility example introduction 

This section is a step by step experiment accessibility rules described in slides 62 and 63.

Start with the following class code:
...
9. During testing, we will concentrate on why a certain expression evaluation succeeds or fails from the accessibility perspective, and will explain the success or failure based on rules in Slides 62 and 63.

  
**** C1: Execute some code in C1, gradually adding to expressions
...
Result: Success/Fails *Here we describe the result of accessibility experiment in the section*
Reason: *Here we describe the reason for the Success/Fails result. In most cases, we tie the reason to the accessibility 'theory', generally in slides 62 and 63 in n [[https://00463064267054129163.googlegroups.com/attach/16b8a25058caa/ns-access-control.pdf?part=0.1&view=1&vt=ANaJVrHvttkWPn-fNP95UFKbcCbbp1VaDJlJSx1HzBevq_xhafVpZ4uz1ffKMTvkAxLFnAk2_xurwU-BTi3FIMnPj1FlU7-S-zKgS2y2eNXheQQsxM8mJUY][Can Beautiful Languages Do Access Control?]]*
...
From this section onward, the method names we run are all public, so they can be executed from accessibility perspective by calling the method on an instance such as ~C1 new~.  The method names attempt to express the expression executed. For example
...
Result: Fails to send message ~protM~
Reason: We showed success all the way before sending ~protM~; sending ~protM~ (fully specified as ~C1>>ProtC2>>#protM~) fails. The code being executed, ~ProtC2 new protM~ is in class ~C1~. Method ~C1>>ProtC2>>#protM~ is a protected member on ~Prot2~, nested in ~C1~. According to *Slide 62 b) Enclosing classes (code in C1) cannot see private or protected members (protected method ~protM~) of nested classes (~ProtC2~).*, and so ~ProtC2 new protM~ fails according to this rule.
...
Does: Creates an instance of class ~ProtC2~ and sends message ~pubM~ to it
Result: Succeeds sending public message ~C1>>ProtC2>>#pubM~ from code in ~C1~
Reason:  ~pubM~ is public. According to [[* Colollary on slides 61, 62, 63][Colollary on slides 61, 62, 63]] public members are accessible anywhere - even in nested classes that are not public. Here, ~C1>>ProtC2>>#pubM~ is accessible from anywhere.   

*This is perhaps the most important discovery so far. Code in class ~C1~ (in it's method ~protC2NewPubM~), can access (call) public method  ~C1>>ProtC2>>#pubM~ on it's nested class, even if the nested class  ~C1>>ProtC2~ is protected - same if the class ~C1>>ProtC2~ was private!! The accessibility rules do not request what I would call 'accessibility along the whole path' - it is sufficient that the accessed member (~C1>>ProtC2>>#pubM~ here) is accessible according to the accessibility rules.*
