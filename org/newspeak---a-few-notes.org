...
- *Module* :   Instance of top level class
...
- *Instance slot declaration* : Expression between the bars ~| |~ locater inside the primary factory method, irrespective whether it has a ~=~, ~=::~ or (nothing) to the right of the slot name. For example ~|mySlot =:: 1|~ is a declaration of a mutable slot named ~mySlot~.  ~|mySlot = 1|~ is a declaration of an immutable slot named ~mySlot~. 
- *Local slot declaration* : Equivalent of a local variable.  Syntactically all above applies, but the expression between the bars ~| |~ is located inside an instance method.
- *Primary factory method* : The method that allows to accept parameters (capabilities), declare instance slots, and produce an instance of a class.  In practical terms, looking at code, the primary factory method is the method starting on the same line where the class code starts. Also, the primary factory method is the only place in which we can declare instance slots:
    #+begin_example
      (* The string "usingPlatform: andA1:" is the primary factory method signature.
         The whole section up to the closing bracket is the factory method.
      *)
...
- *Factory method* : The method that produces an instance but cannot declare instance slots.   In practical terms, looking at code, the factory method is the method in the class code section after the ":" - in this example, section 3. including the surrounding brackets.  
    #+begin_example
      class A2 usingPlatform: p andA1: A1 =
        ...
        (
          (* 2. methods and nested classes *)
        )
        :
        (
          (* 3. factory methods *)
          factoryMethod1 = ( (*Code returning instance*) )
        )
    #+end_example 
...
In the BankAccount video https://www.youtube.com/watch?v=qKWPSvcF0zA, if the exemplar on the factory method 
...
**** Are the exemplars always extracted from the comment before the " = (..)" section defining the factory constructor, instance method or class method, OR can they be at any position in the code?

Exemplars must be placed before the method body starts (or for classes, before factory body starts).

For a concrete example, let us look at this pseudo-code of factory method ~balance: b~, see markings with <== on the right
...
** IN-PROGRESS-NOW Access control and access modifiers in Newspeak.

This section summarizes access control in Newspeak. In other words, what are the rules for class and object members to be allowed access from other classes and objects.  

For a heads up, access control rules define how, if an expression (a line of code), will be valid at runtime. The rules must take into account 

a) access modifiers "private" "public" and default (protected); 
b) if the accessed objects and members are in the same class, nested class or enclosing class of the expression for which the rules evaluate access;
c) if the accessed objects and members are in the same class or an inherited class.

Clearly, access control rules are not trivial. But hopefully we will be able to summarize it reasonably briefly.


*** Introduction to access control in Newspeak

Main article background for this section is the online presentation "Can Beautiful Languages Do Access Control?" https://00463064267054129163.googlegroups.com/attach/16b8a25058caa/ns-access-control.pdf?part=0.1&view=1&vt=ANaJVrHvttkWPn-fNP95UFKbcCbbp1VaDJlJSx1HzBevq_xhafVpZ4uz1ffKMTvkAxLFnAk2_xurwU-BTi3FIMnPj1FlU7-S-zKgS2y2eNXheQQsxM8mJUY.  The text below is based on it and borrows from it, but introduces terminology that is not part of the presentation.

Goal of this section is to understand the Newspeak security and access system.


*** Context to access control in Newspeak.

Let's provide a context of access control.

- Newspeak classes are nested and also inherited. Class members are slots, methods, and their nested classes. 
- Each member may have access modifiers keywords: public, private. No modifier means protected.
- What does "access control" mean in a broader sense? 
   - We want to understand how objects can access ("have the access rights to") other objects and  other objects' members: slots, methods, inner class objects. 
   - The access rights are defined by rules which navigate a path in three independent "dimensions"
...
*** Access control definition and terminology

Let us describe what we mean by the term "Access" or "Access rights" of objA to access objB or objA to send message msg to objB.

- In a Newspeak's (class based) system of objects, each object may contain slots, methods, and nested classes. Because in Newspeak runtime, any operation is a message send, we can speak of the following: 
   ...
   - If objA can reference objB, we say that objA has access (or access rights) to objB. Further, if objA has access to objB, assume there is a method, slot, or nested class msg  declared on objB. Even though objA has access to objB, objA still may or may not be able to send the message msg to objB (execute objB msg). If objA has the ability to send message msg to objB, we say that"object objA has access to msg on objB"
   

Combined, we say that objA has *access* (or *access rights*) to send a message msg to objB, if a message send ~objB msg~ is allowed at runtime from code in objA.

  
*** Slides 61, 62, 63: Rules for accessing members above and below the class in the nesting hierarchy

This section summarizes rules which define whether an object is allowed to send a message to another object.  

- *Slides 61, 62, 63 overview*: They describe how Newspeak code in a class accesses members (classes, slots, methods) above and below the class in the nesting hierarchy and the member members, depending on the accessed member's public, private and protected status. In particular:

- *Slide 61*: describes public, private keywords and protected members of a class. No keyword before the member name makes the member protected.

- *Slide 62*: describes how Newspeak code in a class accesses private and protected members (classes, slots, methods) above and below the class in the nesting hierarchy:

  - a) Private and protected members can be seen by nested classes

  - b) Enclosing classes cannot see private or protected members of nested classes

  - c) Subclasses are never aware of private members of superclasses and vice versa

- *Comments on slide 62*:

  1. Items a) and b) describe lexical access (in terms of nested classes and the enclosing class). We can interpret lexical access as 'looking at source code'. If we print the code of the module declaration (the code of top level class, including all nested classes), and use a ruler to browse up and down the class hierarchy, we are using lexical access.  In other words, think of looking at a line of code with an expression such as ~aObj msg~ or ~msg~ and ask: Is the message send accessible at runtime? Then:

     - The first rule can be interpretted as: If a member ~msg~ exists *UP the lexical scope* as a member, it will be visible (can be accessed at runtime without access error), no matter if it is private, protected or public.

     - The second rule can be interpreted as: If a member ~msg~  exists *DOWN (IN a nested class) the lexical scope*, it will only be accessible if public.

  2. *The Sky and the ground memoization of rules a) and b)*: We can imagine, if we are looking UP, the sky is see-through, everything is visible and accessible, all the way to the remote stars; in our situation all members lexically UP are visible and accessible. Looking DOWN, our visibility of private and protected members stops at seeding the ground - the bottom of the class our line is in. Except the public members which we can always see through the ground.

  3. The third item discusses the rule for inheritance.

- *Slide 63*: discusses how objects can access members. This implies runtime.
...
- *Comments on slide 63*:
  ...
  1. While first two bullets of slide 62 describe how we can evaluate access lexically (looking at code), slide 63 describes runtime: lets say a line ~obj member~ or ~member~ is executed. Slide 63 tells us that:

    - ~obj member~ is valid if member is a member of the object

    - ~member~ is valid if the ~member~ is lexically visible member of an enclosing object (can be several levels up). 

  2. There are further rules which describe how message sends find the object on which the message exists (slides 64-77)
  

*** IN-PROGRESS-NOW todo: Add section with reworded simplified rules 
*** IN-PROGRESS-NOW todo: Worked examples of for slides 61, 62, 63: Rules for accessing members above and below the class in the nesting hierarchy


todo:
-  lexical access have a priority (before inheritance)

*** Email to the Newspeak group

Gilad, 


This is another all too wordy follow up from the earlier thread on access modifiers in Newspeak.


A few Fridays ago, I was on a plane for four hours, and thought I would take an hour to go over the slides in  Can Beautiful Languages Do Access Control?
- https://00463064267054129163.googlegroups.com/attach/16b8a25058caa/ns-access-control.pdf?part=0.1&view=1&vt=ANaJVrHvttkWPn-fNP95UFKbcCbbp1VaDJlJSx1HzBevq_xhafVpZ4uz1ffKMTvkAxLFnAk2_xurwU-BTi3FIMnPj1FlU7-S-zKgS2y2eNXheQQsxM8mJUY


Well it took me the whole four hours and more :) but I feel I got an ok understanding (much better than an earlier "intuitive guessing"). 


General: 


Slide 62 is a great summary of access control that answers some of my core questions about up vs down nesting accessibility. Basically I am thinking of it this way: 
- "I am writing code in the context of class C. If the code needs to use an instance of class B, I need to look at the code of class B with all it's enclosing and nested classes, as if I saved the module declaration where B is declared. The rule is: 
- instance of  B can access all members of all classes in the class B's lexical scope UP;
- instance of B can only access public members of public classes in the class B's lexical scope  DOWN". 


I am not sure if my above interpretation of slide 62 is completely correct, but that is what I came up with after some experimentation.   (Another interpretation of the "going down in nesting" rule would be that, in an access chain such as "objectB NestedInObjectB new nestedInNestedOfObjectB new msg" everything along the access chain" must  be public.)


Also the slides explaining how the lexical chain lookup has precedence to the inheritance chain lookup are super helpful.


Obviously all this is just a part of a necessarily complex problem at the intersection of (or maybe rather on the cartesian product of) access modifiers, nesting, and inheritance.  


One overall comment-question: I see examples in the slides that go over the "comb rule". It is a bit hard without comments for the slides, but I assume the "comb rule" is applied in other languages that have both nested classes and inheritance such as Java (?). The Newspeak rule "lexical scope first, inheritance next" is definitely simpler, but I wonder if detractors would say something like "well in Java we do not really use nested classes all that much, so Java is actually simpler because we have only one dimension, the inheritance axis" ... but in Newspeak nesting is sort of core, so the bigger complexity is inevitable. Anyway, that is just musing.


When reading and thinking about the slides, I wrote up some questions.  I do not expect all to be commented on, for the simple reason my questions' wording may not be comprehensible :) - and they are long ... But I try .. so some questions, trying to set up their context:


- Question 1: My handle of the message send  lookup (slides 64-77) is perhaps decent but shallow.  In particular, it is hard for me to see a motivation for the self sends and implicit receiver sends, regarding the lookup algorithm switching from "look for private msg along the lexical scope dimension first" to "look for protected or public msg along the inheritance dimension next" if the lookup of private msg fails.  Would there be some handy "motivational examples",  as to why such a lookup algorithm (especially around switching from private only to protected or public) was selected, similar to the one example on slides 46 and 47? I feel the result of this decision is "natural" but I am unable to rationalize it or provide a good example.
- Question 2: Is it correct to say that a slot access is a subset of the implicit receiver send? As an example, let's assume in code, there is a line "slot1." or "slot1:: 10." (I mean outside the slot declaration section).  Do we apply the rules defined  for "implicit receiver sends" described in slides 69-77, substituting "msg" for "slot1"?
- Question 3: On slide 88, in the outer send description, should the text be "outer C msg" rather than "outer N msg"? I am asking because the text only talks about C, never about N
- Question 4: On slide 69, implicit receiver send: "Lookup declaration of private msg in immediately surrounding class. If not found, recurse up lexical scope until lexical top. " : What does the "recurse up lexical scope" look for? Does it look up the lexical scope only for private msg? Also if a public or protected msg is found lexically up, is such msg skipped OR does a presence of public or protected msg trigger the inheritance chain lookup, even though there may be lexically higher level private msg?
- Question 5: This is related to the previous question on implicit receiver sends, but expanded to self sends comparison.  What is the difference between the self send lookup and the implicit receiver send lookup ... These two send types are tied nicely in slide 89 'observations' using the outer sends (tied in my understanding -  in the sense of having core similarities, and can be implemented using outer sends).  Would it be reasonable to think of the difference as follows
   - The self send lookup and the implicit receiver send lookup use almost the same algorithm, with one exception: The self send  lookup tries to lookup private msg only in the lexically immediately enclosing class, then the lookup switches over to the inheritance lookup, WHILE the implicit receiver send lookup keeps going lexically all the way up looking for private msg (and only if it fails switches over to inheritance  public/protectedlookup)? 
- A tangent question 5: If I see "self msg." somewhere in code. Can the class of self ever be different from the class that I find by lexical lookup from where the line "self msg." is located? I know I am missing something obvious but can't put my finger on it.
- Question 6: object access to a protected member vs a private member. Slide 63 describes the rule on when an object may access a protected member. If I write the same slide, but replace "protected" with "private", would the slide be correct?
- Question 7: slide 68 'enclosing objects' shows circles with C0, C1 etc. what are they? The classes are S0 ,S1 etc. I assume the O0, O1, O2,etc are instances of S0, S1, S2 et.  Do the C0, C1, C2 etc circles have something to do with showing the fact that classes are virtual in Newspeak?




I would be grateful for any comments but please do not spend too much time parsing my  questions where they are not clear.
...
This is great. I will follow up briefly inline; I think I have a few followup questions between this post and the terminology post, so I will do both in the terminology post. 

After that, only if I still have something to ask after going over the PDF.

On Sunday, 8 August 2021 at 07:53:05 UTC-7 Gilad Bracha wrote:
Hi Milan,

I'm attaching a PDF of slides from a talk I gave in 2015.  Of course, slides without narration are not ideal, but they actually answer most of your questions. There is a lot of general introduction to Newspeak as well, but especially from slide 61 on.
I also answer your questions inline below.

On Sat, Aug 7, 2021 at 11:19 PM Milan Zimmermann <milan.zi...@gmail.com> wrote:
Can I read on visibility modifiers in Newspeak somewhere?

See attachment.

(Hmm, after writing all this I suppose I should start with the specs :) )

Yes, but I acknowledge that is not light reading.

True. I actually went over one of the first versions of the specs back in .. eh, not sure, around 2009 if not before, but it did not register as you see  :) It is terse read without hands on, as it probably should be :)


but still, let me ask about a few things related...

Just playing with simple code, several time I ran into "not understood" because I missed a public keyword on a nested class.

Yes, that happens. What we can and should do is provide better error messages in these cases, indicating that there really was a method, but not accessible. The problem is that from a security perspective, one may not want to make that distinction as it gives out information to an attacker. But during development it would be nice, so it needs thought.

Sounds good. I can see how proper security considerations make developing a platform so much harder, in addition if one wants to toggle behavior on development time vs runtime even more so.
 

- How does visibility work within the class nesting hierarchy, up and down nested classes and their instance methods and slots? 

I call it accessibility, not visibility, to be precise.

I like 'accessibility' and 'access modifiers' fwiw.  Better fitting terms speak meaning more precisely. But at some point in the past I got used to the less used term visibility. I should change the subject of this post. 



- Down direction: Is the code in a nesting class Parent able to access anything or everything in it's nested class Child (slots, classes, constructors, instance and class methods), irrespective of the visibility modifiers anywhere?  

No.  The parent can access the children via "lexical scope" but it has no privileged access to their internals. It accesses them like anyone else.

Ah, ok thanks. I definitely had a misconception here,  I was thinking instances of nesting 'parents' have 'more' access rights to instances of nested 'children' (compared to some other class that got their hands on a child class and instantiated it) . But that would not actually be democratic ... not sure that expresses anything. But for sure I feel to have a better handle after your lexical scope rule answer.


- Up direction: What are the rules in the other direction: Is the code in a nested class Child able to access anything or everything in it's nesting Parent, irrespective of the visibility modifiers anywhere? 

Lexical scoping. This is a bit of a misnomer, as this is really a result of dynamic lookup rules, not static scoping rules, but the effect is the same. This means the child can access anything lexically visible in the enclosing classes (but not in its siblings).

Thanks.

- I see most (all?) constructors on top level classes in the IDE are not public.

No. Top level classes don't have an explicit access modifier, ever, but primary factories are always public. The modifier on a class dictates the classes' accessibility, not the factory method's.

Hence it makes no sense for top  level classes - they are not members of any other object. Once you have access to a class, you can instantiate it (hence factory is always public).

Thanks. I think I will have a follow up question in the other post on this (not to make too many posts running) 



- Regarding visibility rules, is there any difference in a class accessing it's slots, vs it's nested classes, instance and class methods?  

No.

Right, this one I thought was the case, but wanted to check.
