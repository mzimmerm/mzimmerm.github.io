...
<<thing-class-definition>> Let's start with defining class /Thing/ in /CarLibrary/:
...
<<car-class-definition>> Next, let's add a class /Car/:
...
- Paste the code for /Car/ and /Thing/ under TugPresentation
...
******* Class Declaration (line 2)
    ...
    The outer class /CarLibrary/ starting at line 1 and closing at line 23.
...
- In Newspeak: /class Car = Thing ()()/
  ...
  #+BEGIN_SRC
  (* 
     - Repeating the Car constructor from previous section here, with comments removed
  *)
  ...
  class Car color: aColor = Thing (
    ...
    'category misc'
    color = (^ rgbColor, 'converted to String name') (* todo: call some converter From RgbColor To String *)
    printMe       = (^ 'I am a Car, my color is ', color.)
    printColor     = (^ 'My color is ', color.)
  )
  #+END_SRC
...
class CarLibrary = Object()(
  ...
  class RGBCar rgbColor: aRgbColor = Thing (
    ...
    color = (^ rgbColor, 'converted to String name') (* call some converter From RgbColor To String *)
...
Clients may call the RGBCar using either one of the /#color:/ or /#rgbColor:/ constructors, as follows:
...
The ability to take the Car class and create a different version of it, named RGBCar is also an example of /Representation Independence/ in Newspeak. What is meant by that?
- Let's assume we have a class /Car/, with the API defined in [[car-class-definition]].
- Let's assume that /Car/ is imported by, and used by, some client code around the word.
- Let's assume we (the creators of /Car/) want to change internals of /Car/ (reimplement it with different internal state)- that is, we want to change the representation of how color is stored, from /#color/ slot to /#rgbColor/ slot.
- Such representation change is possible in Newspeak as no internal state is exposed except by a message send, which can be adapted to a different internal representation. In the example above, we defined /RGBCar/. After testing /RGBCar/, we can now delete /Car/, and rename /RGBCar/ to /Car/, then start distributing the new /Car/ implementation (representation) to clients. No clients will notice and continue to work, because:
...
1. Clients can still create the /Car/ the same way as before (no need to change the client code):
   ...
   Because a "legacy" /#color:/ constructor was provided as a class method in /RGBCar/ (the new implementation of /Car/).

2. No internals of the (original or new) /Car/ class are exposed except by message send (even internally: If a class addresses it's slot, it does so by a message send!). In other words, if before the change any client was calling

   #+BEGIN_SRC 
   car := (renamed from RGB)Car color: 'blue'.
   carsColor := car color.
   #+END_SRC

   The result in /carsColor/ would be the same before and after we changed the implementation of /Car/ because we provided an adaptor from the old /Car/ /#color/ to the new /(renamed from RGB)Car/ /#color/. There is no client code change.
   

(Altough note that we only pseudo-implemented the /#color:/ method, the result in our case is not 100% the same, but making it the same is just a matter of mechanics of a converter between color and RGB color)
